...
class MetadataDownloader:
    ...
    def process_video(self, entry: dict, dry_run: bool = False) -> bool:
        ...
        if dry_run:
            logger.info(f"[Dry-run] Would download metadata for {video_id}")
            return True

        # Re-fetch full info for this specific video
        try:
            with yt_dlp.YoutubeDL({"quiet": True, "skip_download": True}) as ydl:
                full_info = ydl.extract_info(video_id, download=False)

            out_path = self.config.data_dir / f"{video_id}.json"
            ...
            logger.info(f"Saved metadata: {video_id}.json")
            return True
